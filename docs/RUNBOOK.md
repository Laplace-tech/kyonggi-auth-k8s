...
### 2.2 컨텍스트/네임스페이스 확정 + 이미지 빌드 + import (태그 고정: :local)
```bash
kubectl config use-context k3d-kyonggi
kubectl create ns kyonggi --dry-run=client -o yaml | kubectl apply -f -
...
kubectl -n kyonggi rollout status deploy/backend --timeout=120s
...
```bash
kubectl create ns kyonggi --dry-run=client -o yaml | kubectl apply -f -
kubectl get ns | grep kyonggi
```

### 3.3 시크릿 파일 준비(필수) — 경로 고정
```bash
ls -la deploy/overlays/prod/secrets/backend.env
ls -la deploy/overlays/prod/secrets/mysql.env
```
